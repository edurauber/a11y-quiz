...
const reportContainer=document.createElement('div');reportContainer.className='report-container';reportContainer.innerHTML=`<div class="report-header"><h2>Relatório de Auditoria de Acessibilidade</h2><p>Gerado em: ${timestamp}</p></div>`;const actionsContainer=document.createElement('div');actionsContainer.className='report-actions';actionsContainer.innerHTML=`<button id="copy-button">Copiar (Texto Simples)</button>
...
function closeModal(){modal.classList.add('modal--hidden');modal.setAttribute('aria-hidden','true');document.removeEventListener('keydown',handleKeydown);if(lastFocused&&typeof lastFocused.focus==='function')lastFocused.focus();}
